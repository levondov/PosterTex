...
\node at (14.0,3) {\includegraphics[width=250pt]{Pics/mls.pdf}};
...
\node at (7.5,4){\fbox{\includegraphics[width=100pt]{Pics/beam}}
};
\node at (7.5,2){
Beam used for alignment
};
\node at (17, -0.4){\includegraphics[width=50pt]{Pics/PTB-Logo.pdf}};
...
\node at (-1,-5) {
\begin{varwidth}{7cm}
\color{hzbblue} \textbullet \color{black} The toroidal mirrors, along with a splitter and a normal mirror, on the optical table are placed on rail systems in order to provide multiple degrees of freedom to adjust for the horizontal and vertical focal lengths of the beam
\end{varwidth}
};
\end{tikzpicture}


{\color{hzbblue}\bf\large Testing} \\[0.5cm]
\begin{tikzpicture}[]

\node at (1,0) {\includegraphics[height=100pt]{IP/analog10sx50_backgroundsubstracted.pdf}
};
\node at (6,0) {\includegraphics[height=100pt]{IP/lowalpha_100mA_zerisseneFuellungmitLuecke.pdf}
};
\node at (12,0) {\includegraphics[height=100pt]{IP/lowalpha_1uA.pdf}
};
\node at (4.5,3) {
\Large Horizontal bunch length measurement
};
\node at (1,-1.2) {
old camera
};
